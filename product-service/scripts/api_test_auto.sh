#!/usr/bin/env bash
# api_test_auto.sh - Automated quick validation (CI/CD ready) - UPDATED 2025-11-29
...
echo "=== QUICK API VALIDATION (POST-REFACTOR) ==="
echo ""

# Check server — harus 200 (bukan 204!)
if ! curl -s -o /dev/null -w "%{http_code}" "$BASE_URL/products" | grep -q "200"; then
  echo "[ERROR] Server not running at $BASE_URL"
  exit 1
fi

# Run tests — semua sesuai status code sekarang
...
# Create -> Update -> Patch -> Delete flow
...
if [ -n "$TEST_ID" ] && [ "$TEST_ID" != "null" ]; then
  ...
  test_endpoint "Update product" "PUT"   "/products/$TEST_ID" \
    '{"name":"Updated","price":2000,"qty":10,"category":"test"}' "200"
